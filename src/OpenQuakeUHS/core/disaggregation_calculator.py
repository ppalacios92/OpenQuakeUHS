...
class Disaggregation:
    ...
    def __init__(self, base_path , target_poe , target_imt):
    ...
    def csv_files(self):
        ...
        # === Cargar archivos ===
        data = pd.read_csv(os.path.join(self.base_path, file_data), comment='#')
        data_TRT = pd.read_csv(os.path.join(self.base_path, file_data_TRT), comment='#')  
        self.data =data
        self.data_TRT=data_TRT
    
        return self.data , self.data_TRT
    ...
    def plot_disaggregation(self):
        ...
        ax2.set_title(
            f"Disaggregation Plot by TRT (poe={self.target_poe}, imt={self.target_imt})",
            fontweight='bold'
        )

        # === Leyenda TRT debajo del gráfico 2 ===
        handles_TRT = [mpatches.Patch(color=trt_to_color_TRT[trt], label=trt) for trt in trt_unique_TRT]
        fig.legend(handles=handles_TRT, title='Tectonic Region Type',
